package api

import (
	"context"

	"connectrpc.com/connect"

	apiv1 "github.com/tinkerrc/volunteer/proto/api/v1" // generated by protoc-gen-go
)

// TODO: implement
func (s *APIServer) AddEventVolunteers(
	ctx context.Context,
	req *connect.Request[apiv1.AddEventVolunteersRequest],
) (*connect.Response[apiv1.AddEventVolunteersResponse], error) {
	// u, ok := authn.GetInfo(ctx).(ent.User)
	// if !ok {
	// 	return nil, authn.Errorf("unauthenticated")
	// }

	res := connect.NewResponse(&apiv1.AddEventVolunteersResponse{})
	return res, nil
}
