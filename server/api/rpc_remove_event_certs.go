...
func (s *APIServer) RemoveEventCerts(
	ctx context.Context,
	req *connect.Request[apiv1.RemoveEventCertsRequest],
) (*connect.Response[apiv1.RemoveEventCertsResponse], error) {
	...
	_, err = s.Db.Event.UpdateOneID(eid).RemoveCertIDs(cids...).Save(ctx)
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, errors.New("could not add certs"))
	}
	res := connect.NewResponse(&apiv1.RemoveEventCertsResponse{})
	return res, nil
}
