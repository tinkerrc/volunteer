...
// The VolunteerFunc type is an adapter to allow the use of ordinary
// function as Volunteer mutator.
type VolunteerFunc func(context.Context, *ent.VolunteerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VolunteerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.VolunteerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VolunteerMutation", m)
}
